...
import { encodeBase16, decodeBase16 } from "./codecs";
 ...
 */
const makeRNodeHttpInternal: MakeRNode = () => async (httpUrl, apiMethod, data) => {
...
type SendDeployMethod = (
    r: RNodeHttp,
    n: typeof Date.now
) => (node: { httpUrl: string }, account: RevAccount, code: string, phloLimit?: number) => Promise<Deploy>;

/**
 * Creates signed deploy.
 */
const getSignedDeploy: SendDeployMethod =
    (rnodeHttp, now) =>
    async ({ httpUrl }, account, code, phloLimit) => {
        ...
        const deploy = account.privKey ? signPrivKey(deployData, account.privKey) : await signMetamask(deployData);
 ...
 */
const sendDeploy: SendDeployMethod =
    (rnodeHttp, now) =>
    async ({ httpUrl }, account, code, phloLimit) => {
        // Check if deploy can be signed
        if (!account.privKey) {
...
let GET_DATA_TIMEOUT_HANDLE: ReturnType<typeof setTimeout>;

type DataForDeployMethod = (
    r: RNodeHttp
) => (node: RNodeHttpUrl, deployId: string, onProgress: () => boolean) => Promise<{ data: any; cost: number }>;

/**
 * Listen for data on `deploy signature` (`rho:rchain:deployId`).
 */
const getDataForDeploy: DataForDeployMethod =
    (rnodeHttp) =>
...
type FetchDeployMethod = (r: RNodeHttp) => (node: RNodeHttpUrl, deployId: string) => Promise<DeployResult>;

/**
 * Get deploy result from the block where is proposed (throws error if not found).
 */
const fetchDeploy: FetchDeployMethod =
    (rnodeHttp) =>
