...
import { IPFS_RESOLVER_VIDEO, IPFS_RESOLVER_IMAGE, RESIZER_IMAGE } from '../../utils/constants'
...
const TemplateCard = ({
    ...
    useEffect(() => {
        if (Date.now() - 600000 < Number(createdAt) && isMyTemplate) {
            ...
            const fallbackImageSrc =
                !noIpfsConversion && templateImage ? `${IPFS_RESOLVER_IMAGE}${templateImage}` : ''
    ...
    const isMyTemplate = currentUser && router.query.chainAccount === currentUser.actor
    ...
    const collectionIcon = hasPlaceholderIcon ? (
        ...
        <CollectionIcon name={collectionName} image={collectionImage} margin="24px 16px 24px 0" />
    )
    ...
    return (
        ...
        >
            ...
            {templateVideo ? (
                <TemplateVideo src={templateVideoSrc} priceTag={priceTag} autoPlay={autoPlay} />
            ) : (
                <TemplateImage
            ...
            <GreyText>Edition size: {maxSupply === '0' ? 'Unlimited' : maxSupply}</GreyText>
