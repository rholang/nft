...
export type RNodeHttp = (
  httpUrl: string,
  apiMethod: string,
  data?: any
) => Promise<any>;

export type RNodeWebAPI = SendDeployEff &
  GetDeployDataEff &
  ProposeEff &
  RawRNodeHttpEff &
  GetSignedDeployEff;
 ...
 */
const makeRNodeHttpInternal: MakeRNode =
  () => async (httpUrl, apiMethod, data) => {
  ...
  async ({ httpUrl }, account, code, phloLimit) => {
    ...
    if (!account.privKey) {
      ...
      if (ethDetected && !!selEthAddr) {
        ...
        if (
          ethAddr.replace(/^0x/, "").trim().toLowerCase() !==
          selEthAddr.trim().toLowerCase()
        )
...
const getDataForDeploy: DataForDeployMethod =
  ...
  async ({ httpUrl }, deployId, onProgress) => {
    ...
    const getData =
      (resolve: (d: any) => void, reject: (ex: Error) => void) => async () => {
...
type FetchDeployMethod = (
  r: RNodeHttp
) => (node: RNodeHttpUrl, deployId: string) => Promise<DeployResult>;
...
type ProposeMethod = (
  r: RNodeHttp
) => (node: RNodeHttpAdminUrl) => Promise<string>;
 ...
 */
export const signPrivKey = function (
